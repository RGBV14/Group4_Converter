...
def N2C():
    def calc_nines_complement():
        output_text_widget.delete('1.0', END)
        output_text_widget.insert(END, nines_complement(input_text_widget.get()))

    N2Cwin = Tk()
    N2Cwin.title("9's Complement Converter")

    frame_outer = Frame(N2Cwin, bg='black')
    ...
    execute_widget = Button(f2, text='    Convert   ', font=('arial', 15, 'bold'), command=calc_nines_complement)
...
def nines_complement(number):
    ...
    try:
        number = int(number)
        return ''.join(str(9 - int(digit)) for digit in str(number))
    except ValueError:
        return "Invalid input"



#function for decimal to 10s complement

def tens_complement(number):
    num_str = str(number)
    complement = ''.join(str(9 - int(digit)) for digit in num_str)
    return str(int(complement) + 1)


def calculate_tens_complement():
    input_num = input_text_widget.get()
    if input_num.isdigit():
        result = tens_complement(input_num)
        output_text_widget.delete('1.0', END)
        output_text_widget.insert(END, result)
    else:
        output_text_widget.delete('1.0', END)
        output_text_widget.insert(END, "Invalid input")

    # create the main window


win = Tk()
win.title("10's Complement Calculator")
col = 'gray80'

frame_outer = Frame(win, bg='black')
frame_outer.pack(fill='both', expand=True, padx=5, pady=5)
...
l0 = Label(f1, font=('arial', 25, 'bold'), text='Input:', bg=col)
input_text_widget = Entry(f2)
execute_widget = Button(f2, text='    Calculate   ', font=('arial', 15, 'bold'), command=calculate_tens_complement)
l1 = Label(f3, font=('arial', 25, 'bold'), text='Output:', bg=col)
output_text_widget = Text(f4, width=45, height=1)
...
submenu.add_separator()
submenu.add_cascade(label='Decimal to 9s complement', command= N2C)
submenu.add_cascade(label='Decimal to 10s complement', command= N2C)
submenu.add_separator()
